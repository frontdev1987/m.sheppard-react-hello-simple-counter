import PropTypes from "prop-types";
import React from "react";

const SimpleCounter = (props) => {
  return (
    <div
      style={{
        display: "flex",
        justifyContent: "center",
        alignItems: "center",
      }}
    >
      <div className="clock-img">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
          fill="currentColor"
          className="bi bi-clock"
          viewBox="0 0 16 16"
        >
          <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71z" />
          <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16m7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0" />
        </svg>
      </div>
      <div
        className="seconds hundred-thousands"
        style={{
          backgroundColor: "black",
          color: "white",
          padding: "20px",
          margin: "5px",
          fontSize: "2rem",
          borderRadius: "5px",
          width: "60px",
          height: "80px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        {props.hundredThousandsDigit}
      </div>
      <div
        className="seconds ten-thousands"
        style={{
          backgroundColor: "black",
          color: "white",
          padding: "20px",
          margin: "5px",
          fontSize: "2rem",
          borderRadius: "5px",
          width: "60px",
          height: "80px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        {props.tenThousandsDigit}
      </div>
      <div
        className="seconds thousands"
        style={{
          backgroundColor: "black",
          color: "white",
          padding: "20px",
          margin: "5px",
          fontSize: "2rem",
          borderRadius: "5px",
          width: "60px",
          height: "80px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        {props.thousandsDigit}
      </div>
      <div
        className="seconds hundreds"
        style={{
          backgroundColor: "black",
          color: "white",
          padding: "20px",
          margin: "5px",
          fontSize: "2rem",
          borderRadius: "5px",
          width: "60px",
          height: "80px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        {props.hundredsDigit}
      </div>
      <div
        className="seconds tens"
        style={{
          backgroundColor: "black",
          color: "white",
          padding: "20px",
          margin: "5px",
          fontSize: "2rem",
          borderRadius: "5px",
          width: "60px",
          height: "80px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        {props.tensDigit}
      </div>
      <div
        className="seconds ones"
        style={{
          backgroundColor: "black",
          color: "white",
          padding: "20px",
          margin: "5px",
          fontSize: "2rem",
          borderRadius: "5px",
          width: "60px",
          height: "80px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        {props.onesDigit}
      </div>
    </div>
  );
};

SimpleCounter.propTypes = {
  hundredThousandsDigit: PropTypes.number,
  tenThousandsDigit: PropTypes.number,
  thousandsDigit: PropTypes.number,
  hundredsDigit: PropTypes.number,
  tensDigit: PropTypes.number,
  onesDigit: PropTypes.number,
};

export default SimpleCounter;
